...
function openContactInfo(){
  const previous=document.getElementById('contactInfoModal');
  if(previous) previous.remove();
  const modal=document.createElement('div');
  modal.className='modalbg open';
  modal.id='contactInfoModal';
  modal.innerHTML=`<div class="modal wide-modal section-info-modal contact-info-modal">
    <div class="modal-head"><div><h3>Contacto Ducks Basketball Academy</h3><small>Datos de atención, horario y ubicación</small></div><button class="btn secondary" onclick="closeModal('contactInfoModal')">Cerrar</button></div>
    <div class="modal-body">
      <div class="section-info-grid contact-info-grid">
        <article><h4>WhatsApp</h4><p><b>${esc(ACADEMY_WHATSAPP)}</b><br>Atención para informes, pagos e inscripción.</p></article>
        <article><h4>Horario</h4><p>${esc(ACADEMY_HOURS)}</p></article>
        <article><h4>Ubicación</h4><p>${esc(ACADEMY_ADDRESS)}</p></article>
      </div>
      <div class="actions contact-info-actions">
        <button class="btn secondary" onclick="window.open('${ACADEMY_MAP_URL}','_blank')">Ver ubicación</button>
        <button class="btn green" onclick="closeModal('contactInfoModal');openDucksWhatsApp('Hola, quiero pedir más información de Ducks Basketball Academy.')">Pedir más información</button>
      </div>
    </div>
  </div>`;
  document.body.appendChild(modal);
...
function openPublicMoreMenu(){
  ...
  modal.innerHTML=`<div class="modal public-more-modal"><div class="modal-head"><div><h3>Más opciones</h3><small>Ducks Basketball Academy</small></div><button class="btn secondary" onclick="closeModal('publicMoreModal')">Cerrar</button></div><div class="modal-body"><div class="public-more-grid">
    ...
    <button onclick="closeModal('publicMoreModal');openContactInfo()">☎️ Contacto</button>
  ...
  app.innerHTML=`<div class="public-site ducks-mockup-home">
    <main class="ducks-mockup-wrap">
      <section id="inicio" class="ducks-mockup-screen" aria-label="Portada principal Ducks Basketball Academy">
        ...
        <button class="mockup-hotspot hs-card hs-card-3" type="button" aria-label="Contacto" onclick="openContactInfo()"></button>
...
function cashReceiptHtml(data){
  ...
  return `<section class="cash-receipt-print cash-receipt-formal" id="cashReceiptPrintable">
    ...
    <div class="cash-receipt-body">
      ...
      <div class="cash-receipt-stamp-only-block">
        <div class="cash-receipt-stamp-only">${data.use_stamp?`<img class="cash-receipt-stamp-image" style="transform:rotate(${stampAngle}deg)" src="${esc(data.stamp_url||officialReceiptStampUrl())}" alt="Sello oficial de pago recibido">`:''}<div><strong>${esc(data.received_by||'Administración Ducks')}</strong><span>Validación con sello oficial de pago recibido</span></div></div>
        <div class="cash-receipt-verification"><strong>${esc(data.validation||'Validado desde sesión administrativa')}</strong><span>${esc(data.generated_at||'')}</span></div>
      </div>
